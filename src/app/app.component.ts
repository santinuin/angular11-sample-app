...
export class AppComponent implements OnInit, OnDestroy {
  title = 'Ejemplo SPA [Angular 11 - Angular v2]';
  isIframe = false;
  loginDisplay = false;
  ...
  ngOnInit(): void {
    //Agregado para visualizar el token de acceso en un decodificador
    this.authService.handleRedirectObservable().subscribe({
      next: (result: AuthenticationResult) => {
        if (result){
          this.authService.instance.setActiveAccount(result.account);
          console.log('Acces token', result.accessToken);
          console.log('ID token', result.idToken);

          window.open(`https://jwt.ms/#access_token=${result.accessToken}`, '_blank');
        }
      },
      error: (error) => {
